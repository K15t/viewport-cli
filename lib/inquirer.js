...
// ToDo: clarify what configurations do, e.g. scope, clarify what validation expects
// let defaultConfig default to empty object instead of undefined so accessing non-existing properties doesn't throw
async function askConfig({existingEnvNames = [], defaultConfig = {}, envTemplate}) {
    return inquirer.prompt([
        {
            name: 'envName',
            type: 'input',
        ...
        },
        {
            name: 'scope',
            type: 'input',
            message: 'Enter space key to scope (empty for global)',
            default: defaultConfig.scope || '',
            validate: value => envTemplate['scope'].test(value) || 'Enter a valid scope. It may contain up to 225 alphanumeric characters. NOTE: Scroll Viewport treats space keys case-sensitive even though for Confluence they are case-insensitive!'
        },
    ]);
}
